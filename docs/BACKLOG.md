...
- [ ] **Phase 1b — Electron wrapper (self-implementation)** — Ken implements independently using Claude Code; main.js + packaging config only, React unchanged; see GitHub Issue #13 for full task checklist (est. 2–3 sessions)
  - Candidate build tool: electron-vite (github.com/alex8088/electron-vite, MIT license, 5.5k stars, actively maintained as of June 2026). Provides built-in V8 bytecode source protection plus simplified main/preload/renderer config versus hand-rolling main.js. Strong adoption signals make this a likely yes — but confirm two things at Phase 1b start: (1) pin to latest stable release, not a beta tag, (2) treat V8 bytecode as light obfuscation only, not strong IP protection — it raises the bar above plaintext but does not stop a determined reverse-engineering effort. Do not commit to this tool in CLAUDE.md until Phase 1b begins.
